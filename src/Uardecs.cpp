...
const char *UECSattrChar[] = { UECSccm_ROOMTXT, UECSccm_REGIONTXT, UECSccm_ORDERTXT, UECSccm_PRIORITYTXT,};
    ...
    for(int i = startid; i < U_MAX_CCM; i++){
      if(U_ccmList[i].ccmLevel == NONE || U_ccmList[i].sender == true){continue;}
        ...
        if(U_orgAttribute.flags&ATTRFLAG_ALLOW_ABRIDGE_TYPE)
        	...
        	*/
        if(strcmp(UECStempStr20, _tempCCM->type) != 0){continue;}
...
void HTTPGetFormDataLANSettingPage()
{
	...
	for(i=0;i<19;i++)
	{
		...
		if(i<18 && (UECStempValue[i]<0 || UECStempValue[i]>255)){return ;}//IP address,room,region
		else if (UECStempValue[i]<0 || UECStempValue[i]>30000){return ;}//order
	}
       ...
       for(int i = 0; i < 20; i++)
		...
		if(UECSbuffer[startPos+i]=='<' || UECSbuffer[startPos+i]=='>')//eliminate tag
			...
			if(UECSbuffer[startPos+i]=='\0' || i==19){return;}//I’[‚ª–³‚¢‚Ì‚Å–³Ž‹
...
//find first number
for(; (*lastPos) < _targetBuffersize; (*lastPos)++) // â† æ¡ä»¶ã‚’ä¿®æ­£
	{
